...
public class DbOpsEventListner implements EventListner, LifeCycle {
    ...
    public boolean isEventOfInterest(Event evt) {
       boolean ret =  ((evt instanceof TxnEvent) && ((TxnEvent) evt).getTableName().equalsIgnoreCase(tableName));
       log.log(Level.FINEST, "isEventOfInterest {0}, Event Class [{1}], Table Name [{2}] ",  new Object[]{ret, evt.getClass().getName(), ((TxnEvent) evt).getTableName()});
       return ret;
    }
