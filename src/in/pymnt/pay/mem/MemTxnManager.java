...
*/
package in.pymnt.pay.mem;

import in.pymnt.event.EventCentral;
import in.pymnt.pay.event.AccountInfoUpdatedEvent;
import in.pymnt.pay.event.BalanceUpdatedEvent;
import in.pymnt.pay.event.TxnOccuredEvent;
import in.pymnt.pay.txn.AccountInfo;
import in.pymnt.pay.txn.AccountInfoManager;
import in.pymnt.pay.txn.DbIterator;
import in.pymnt.pay.txn.Balance;
import in.pymnt.pay.txn.TxnException;
import in.pymnt.pay.txn.TxnFactory;
import in.pymnt.pay.txn.TxnManager;
...
public class MemTxnManager implements TxnManager, DbIterator, AccountInfoManager, TxnFactory {
    ...
    public void updateBalance(Balance account) throws TxnException {
        balMap.put(account.getAccountName(), account);
         EventCentral.publish(new BalanceUpdatedEvent(account));
    }

    @Override
    public void recordTxn(TxnPayload payload) throws TxnException {
        txnArr.add(payload);
         EventCentral.publish(new TxnOccuredEvent(payload));
    }
    ...
    public void updateAccountInfo(AccountInfo act) {
        ...
        EventCentral.publish(new AccountInfoUpdatedEvent(act));
    }
